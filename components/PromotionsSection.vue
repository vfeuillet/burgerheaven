...
const query = `
  query {
    produits(filters: { enPromotion: { eq: true } }, pagination: { limit: 100 }) {
      documentId
      nom
      description
      prix
...
const { data, pending, error } = await useAsyncData<GqlResponse>(
  'produits-promo',
  () => useGql<GqlResponse>(query),
  { server: true }
)

const produitsPromo = computed<Produit[]>(() => data.value?.data?.produits ?? [])
  ...
  <p style="font-size: 1.125rem; color: #4b5563; max-width: 48rem; margin: 0 auto;">
    Profitez de nos offres exceptionnelles avant qu'il ne soit trop tard !
      ...
      >
        ...
        >
          <!-- Badge PROMO (seulement si prixPromo existe) -->
          <div 
          ...
          <!-- Image produit (hauteur fixe 192px) -->
          ...
          <div style="padding: 1.5rem;">
            ...
            <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 0.75rem;">
              ...
              <div style="text-align: right; margin-left: 1rem; flex-shrink: 0;">
                <!-- Si prixPromo existe, afficher prix barré -->
            ...
            <!-- Badge économie (seulement si prixPromo) -->
            <div 
              v-if="item.prixPromo && item.prix"
